...
defmodule Anubis.Server.Transport.StreamableHTTP.ResumabilityTest.FailingDeleteStore do
  @moduledoc false
  @behaviour Anubis.Server.Transport.StreamableHTTP.EventStore
  ...
  @impl true
  def child_spec(opts), do: InMemory.child_spec(opts)
  @impl true
  defdelegate append(name, session_id, data), to: InMemory
  @impl true
  defdelegate replay(name, session_id, after_id), to: InMemory
  @impl true
  defdelegate latest_id(name, session_id), to: InMemory
  @impl true
  def delete(_name, _session_id), do: {:error, :boom}
end
...
defmodule Anubis.Server.Transport.StreamableHTTP.ResumabilityTest do
  ...
  alias Anubis.Server.Transport.StreamableHTTP.ResumabilityTest.FailingDeleteStore
  ...
  describe "transport recording" do
    ...
    test "an append failure is not delivered as a mis-numbered legacy event" do
      ...
      assert :ok = StreamableHTTP.register_sse_handler(transport, session)
      # The append failure is surfaced to the caller as an explicit partial
      # result naming the failed session, rather than reported as a phantom
      # success, and nothing is delivered with a fabricated legacy id.
      assert {:error, {:partial_broadcast, [{^session, :boom}]}} =
               StreamableHTTP.send_message(transport, "dropped", timeout: 5_000)

      refute_receive {:sse_message, _}, 50
      refute_receive {:sse_message, _, _}, 50
    end

    test "close_session_stream fails closed when the store cannot delete" do
      store_name = start_store()
      task_sup = :"task_sup_#{System.unique_integer([:positive])}"
      start_supervised!({Task.Supervisor, name: task_sup})
      ...
      {:ok, transport} =
        start_supervised(
          {StreamableHTTP,
           ...
           name: :"transport_#{System.unique_integer([:positive])}",
           task_supervisor: task_sup,
           event_store: {FailingDeleteStore, store_name},
           keepalive: false},
          id: :failing_delete_transport
        )
      ...
      assert {:error, :boom} = StreamableHTTP.close_session_stream(transport, session)

      # Events stay in the store and the stream stays open, so a retried
      # DELETE can complete the cleanup instead of silently losing it.
      assert {:ok, [{1, "e1"}]} = InMemory.replay(store_name, session, 0)
    end
  end
  ...
  describe "Plug DELETE with failing event-store deletion" do
    setup do
      store_name = start_store()
      transport_name = Registry.transport_name(StubServer, :streamable_http)
      task_sup = Registry.task_supervisor_name(StubServer)
    ...
    test "DELETE fails closed with 500 when the stream cannot be deleted", %{opts: opts} do
      conn =
        :delete
        |> conn("/")
        |> put_req_header("mcp-session-id", "delete-500-session")
