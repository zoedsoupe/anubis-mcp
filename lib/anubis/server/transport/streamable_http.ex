defmodule Anubis.Server.Transport.StreamableHTTP do
  ...
  def close_session_stream(transport, session_id) do
    GenServer.call(transport, {:close_session_stream, session_id})
  end
  ...
  @impl GenServer
  def handle_call({:close_session_stream, _session_id}, _from, %{event_store: nil} = state) do
    {:reply, :ok, state}
  end

  def handle_call({:close_session_stream, session_id}, _from, %{event_store: {mod, name}} = state) do
    timers = cancel_close_timer(state.stream_timers, session_id)
    state = %{state | stream_timers: timers}

    case mod.delete(name, session_id) do
      :ok ->
        {:reply, :ok, %{state | streams: MapSet.delete(state.streams, session_id)}}

      {:error, _reason} = error ->
        log_delete_result(error, session_id)
        {:reply, error, state}
    end
  end
  ...
  defp broadcast(%{event_store: {_mod, _name} = store} = state, message) do
    ...
    case failures do
      [] -> :ok
      _ -> {:error, {:partial_broadcast, Enum.reverse(failures)}}
    end
  end
  ...
  defp find_handler_session(sse_handlers, pid, ref) do
    Enum.find_value(sse_handlers, fn {session_id, {handler_pid, monitor_ref, _meta}} ->
